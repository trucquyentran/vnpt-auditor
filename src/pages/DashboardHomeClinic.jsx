...
export default function DashboardHomeClinic() {
  const {
    activeSession,
    currentSessionData,
    selectedArea,
  ...
  const giaHan2026ChartData = useMemo(() => {
    const { byAreaMonthGiaHanSom2026, byAreaMonthGiaHanDungHan2026 } = giaHan2026ByAreaMonth;
  ...
  const totalsGiaHanChart = useMemo(() => {
    ...
    return { totalHetHan2026, totalGiaHanDungHan2026, totalGiaHanSom2026 };
  }, [giaHan2026ChartData]);
  ...
  const totalGiaHan2026HC = totalsLocal.giaHanDungHan2026 + totalsLocal.giaHanSom2026;
    ...
    <div className="space-y-4 animate-in fade-in duration-500 pb-10 text-[12px] w-full">
              ...
              <div className="flex items-center justify-center gap-3 mt-1 flex-1 min-h-0">
                <div className="flex items-center justify-center shrink-0">
                  <h2 className="text-2xl font-black text-slate-800">
                    {totalGiaHan2026HC.toLocaleString()}
                  </h2>
                </div>
                <div className="flex-1 min-w-0 text-left text-[9px] text-slate-500 space-y-0.5 font-medium">
                  <span className="block">Đúng hạn: <span className="font-black text-slate-700 text-[9px]">{totalsLocal.giaHanDungHan2026.toLocaleString()}</span></span>
                  <span className="block">Sớm: <span className="font-black text-slate-700 text-[9px]">{totalsLocal.giaHanSom2026.toLocaleString()}</span></span>
                </div>
              </div>
            </div>
          ) : (
      ...
      {!selectedArea && giaHan2026ChartAreas.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden p-4">
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-[11px] font-bold justify-center">
            <span className="text-rose-600">
              Tổng thuê bao chưa gia hạn:{" "}
              <strong className="text-rose-700">{totalsGiaHanChart.totalHetHan2026.toLocaleString()}</strong> TB
            </span>
            <span className="text-emerald-600">
              Tổng thuê bao gia hạn đúng hạn:{" "}
              <strong className="text-emerald-700">{totalsGiaHanChart.totalGiaHanDungHan2026.toLocaleString()}</strong> TB
            </span>
            <span className="text-sky-600">
              Tổng thuê bao gia hạn sớm:{" "}
              <strong className="text-sky-700">{totalsGiaHanChart.totalGiaHanSom2026.toLocaleString()}</strong> TB
            </span>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden p-4">
          ...
          <div className="h-[380px] w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={ptmChartData}
                margin={isNarrowChart ? { top: 40, right: 16, left: 8, bottom: 170 } : { top: 40, right: 16, left: 0, bottom: 140 }}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                <XAxis
                  dataKey="khuVuc"
